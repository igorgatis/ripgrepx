    ...
    json!({"tools": [
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "weights": {"type": "string", "description": "for sort=weight: declare branch weights as label:weight,... (e.g. \"impl:0.7,call:0.3\") and tag regex alternation branches in `pattern` with <label> (e.g. \"fn (process<impl>|process\\\\(<call>)\"). Each file ranks by the highest-weighted branch it matched (a match in no tagged branch scores 0 and sorts last). Weights are relative ranks -- any finite numbers, larger first, not probabilities (needn't sum to 1). The <label> tags are stripped before searching, so this never changes which files/lines match (the set stays ripgrep's) -- it only reorders."},
